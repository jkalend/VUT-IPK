 ...
 * IPK.2015L
 *
 * Demonstration of  TCP server implementation using select.
 *
...
int main (int argc, const char * argv[]) {
	...
	while(1)
	{
		...
		if (comm_socket > 0)
		{
            // Nastaveni schranky do neblokujiciho rezimu:
			...
			{				
                fd_set fds;
                FD_ZERO(&fds);
                FD_SET(comm_socket, &fds);
                FD_SET(STDIN_FILENO, &fds);	
				if (select(comm_socket + 1, &fds, NULL, NULL, NULL) <= 0) continue;     
                                                                   
                printf("*");
                /* Cteni ze schranky */
				if (FD_ISSET(comm_socket, &fds))
				{
                ...
                if (FD_ISSET(STDIN_FILENO, &fds))
                {
                    ...
                    send(comm_socket, input, strlen(input), 0);        
                }
			}
		}
	}	
