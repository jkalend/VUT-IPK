...
def fixture_server_tcp():
    process = subprocess.Popen([find_file("ipkcpd", os.getcwd()), "-p", "2023", "-h", "localhost", "-m", "tcp"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE)
    yield process
...
def fixture_server_udp():
    process = subprocess.Popen([find_file("ipkcpd", os.getcwd()), "-p", "2023", "-h", "localhost", "-m", "udp"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE)
    yield process
    process.send_signal(subprocess.signal.SIGINT)
